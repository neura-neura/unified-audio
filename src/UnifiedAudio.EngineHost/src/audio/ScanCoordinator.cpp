                         ...
                         juce::String* crashCode)
{
    ...
    if (xml == nullptr || ! xml->hasTagName ("UnifiedAudioScanResult")) return false;
                                          ...
                                          const juce::StringArray& forceRescan)
{
    ...
    for (int i = skipped.size(); --i >= 0;)
    {
        ...
        if (! f.exists()
            || forceRescan.contains (skipped[i].file)
            || effectiveModTime (f).toMilliseconds() != skipped[i].fileTimeMs)
        {
...
PluginBinaryFingerprint fingerprintPluginBinary (const juce::File& vst3FileOrBundle)
{
    PluginBinaryFingerprint result;
    result.file = vst3FileOrBundle.getFullPathName();
    if (! vst3FileOrBundle.exists()) return result;

    juce::StringArray binaryPaths;
    if (vst3FileOrBundle.isDirectory())
        for (const auto& binary : vst3FileOrBundle.findChildFiles (juce::File::findFiles, true, "*.vst3"))
            binaryPaths.add (binary.getFullPathName());
    else
        binaryPaths.add (vst3FileOrBundle.getFullPathName());
    binaryPaths.sort (true);

    juce::String digestMaterial;
    for (const auto& path : binaryPaths)
    {
        const juce::File binary (path);
        const auto bytes = binary.getSize();
        result.totalBytes += bytes;
        const auto relativePath = vst3FileOrBundle.isDirectory()
            ? binary.getRelativePathFrom (vst3FileOrBundle)
            : binary.getFileName();
        digestMaterial << relativePath << "\n" << juce::String (bytes) << "\n"
                       << juce::SHA256 (binary).toHexString() << "\n";
    }
    if (binaryPaths.isEmpty())
    {
        result.totalBytes = vst3FileOrBundle.getSize();
        digestMaterial << vst3FileOrBundle.getFileName() << "\n"
                       << juce::String (result.totalBytes) << "\n";
    }
    result.sha256 = juce::SHA256 (digestMaterial.toUTF8()).toHexString();
    return result;
}

PluginFingerprintCoordinator::PluginFingerprintCoordinator (
    juce::StringArray filesToAudit,
    juce::Array<PluginBinaryFingerprint> cachedFingerprints,
    std::function<void (PluginFingerprintAuditOutcome)> onFinished)
    : juce::Thread ("PluginFingerprintAudit"),
      files (std::move (filesToAudit)),
      cached (std::move (cachedFingerprints)),
...
void PluginFingerprintCoordinator::run()
{
    PluginFingerprintAuditOutcome outcome;
    for (const auto& path : files)
    {
        if (threadShouldExit()) return;
        auto fingerprint = fingerprintPluginBinary (juce::File (path));
        const auto cachedEntry = std::find_if (cached.begin(), cached.end(), [&] (const auto& candidate)
        {
            return candidate.file.equalsIgnoreCase (path);
        });
        if (cachedEntry == cached.end()
            || cachedEntry->totalBytes != fingerprint.totalBytes
            || cachedEntry->sha256 != fingerprint.sha256)
            outcome.changed.add (path);
        outcome.current.add (std::move (fingerprint));
    }
    outcome.completed = true;
    if (finishedCb)
        juce::MessageManager::callAsync ([alive = alive, callback = finishedCb, outcome]
        {
            if (*alive) callback (outcome);
        });
}
...
namespace
{
    // Startet UnifiedAudio.exe --scan <plugin> --out <tmpfile> und wartet in 100-ms-Schritten,
