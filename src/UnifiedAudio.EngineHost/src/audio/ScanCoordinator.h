...
// Injizierbar für Tests. Die echte Implementierung startet "UnifiedAudio.exe --scan".
...
// Parst die vom Kindmodus geschriebene Ergebnisdatei (<UnifiedAudioScanResult> mit PLUGIN-
...
// Hashes only the loadable .vst3 binary (or binaries inside a bundle), never logs,
// presets or mutable content. Size and SHA-256 are both persisted in the scan cache.
PluginBinaryFingerprint fingerprintPluginBinary (const juce::File& vst3FileOrBundle);

struct PluginFingerprintAuditOutcome
{
    juce::Array<PluginBinaryFingerprint> current;
    juce::StringArray changed;
    bool completed = false;
};

class PluginFingerprintCoordinator final : private juce::Thread
{
public:
    PluginFingerprintCoordinator (juce::StringArray filesToAudit,
                                  juce::Array<PluginBinaryFingerprint> cachedFingerprints,
                                  std::function<void (PluginFingerprintAuditOutcome)> onFinished);
    ~PluginFingerprintCoordinator() override;

private:
    void run() override;
    juce::StringArray files;
    juce::Array<PluginBinaryFingerprint> cached;
    std::function<void (PluginFingerprintAuditOutcome)> finishedCb;
...
struct UnifiedAudio3Format : juce::VST3PluginFormat
{
